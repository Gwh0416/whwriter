...
- **Agent Wiki 长期记忆**：将角色、地点、物品、事件和伏笔组织为带别名、时效关系与原文证据的知识图；支持力导向关系图、章节视图和节点逐层展开，再以 SQLite FTS5/BM25 补充长设定。
...
章节完成后自动提炼并更新小说运行状态。工作台可查看当前状态、Agent Wiki 实体页、关系与事件时间线、长期设定、伏笔、章节摘要、基础文件和历史快照。

![记忆文件：当前状态、角色、设定、伏笔与章节快照](docs/images/07-memory-files.png)

### Agent Wiki：知识图谱

Agent Wiki 将角色、地点、物品、组织、事件、伏笔和规则统一为实体，并用带章节有效期的关系边连接。写作时先根据本章指令和当前状态定位种子实体，展开一跳关系与关联事件，再使用 FTS5/BM25 补充长文本证据。

- **Wiki 页**：按实体查看别名、关系历史、原文证据和关联事件。
- **关系图**：使用力导向布局展示实体与关系，支持拖拽、缩放、类型筛选和视图适配。
- **章节视图**：切换章节后，只展示该章时间点仍然有效的关系。
- **渐进展开**：默认展示有关联的概览节点，也可围绕选中实体继续展开一跳，避免全量图谱失去可读性。

![Agent Wiki：力导向关系图、章节视图与实体详情](docs/images/08-agent-wiki-graph.png)
...
```text
本章指令 + 当前状态
  -> Agent Wiki：别名解析种子实体，展开一跳关系与关联事件
  -> FTS5/BM25：补充相关基础设定、摘要与证据
  -> Planner：生成本章备忘录
  -> Composer：构建上下文包与规则栈
  -> Writer：生成正文
  -> Auditor：检查连续性和质量问题
...
```bash
cd backend
go run -tags sqlite_fts5 ./cmd/whwriter
```
...
```bash
# 后端测试
cd backend && go test -tags sqlite_fts5 ./...
...
- 前端：Vue 3、Vite、Vue Router、Cytoscape.js
