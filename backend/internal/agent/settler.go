...
type SettlerAgent struct{}

func NewSettlerAgent() *SettlerAgent {
	return &SettlerAgent{}
}

func (a *SettlerAgent) Name() string { return "settler" }

type SettlerInput struct {
	Title           string
	GenreName       string
	Genre           string
	Platform        string
...
func (a *SettlerAgent) BuildSystemPrompt(in SettlerInput) string {
	var b strings.Builder

	b.WriteString(`你是状态追踪分析师。给定新章节正文和当前 truth 文件，你的任务是产出更新后的 truth 文件。
...
- 标题：`)
	...
	b.WriteString(fmt.Sprintf("\n- 题材：%s（%s）\n- 平台：%s\n", in.GenreName, in.Genre, in.Platform))

	if in.NumericalSystem {
		b.WriteString("- 本题材有数值/资源体系，你必须在 UPDATED_LEDGER 中追踪正文中出现的所有资源变动\n- 数值验算铁律：期初 + 增量 = 期末，三项必须可验算\n")
	} else {
		b.WriteString("- 本题材无数值系统，UPDATED_LEDGER 留空\n")
	}

	b.WriteString(settlerHookRules)
	b.WriteString(settlerOutputFormat)
	b.WriteString(settlerKeyRules)

	return b.String()
}

func (a *SettlerAgent) SystemPrompt() string {
	return settlerKeyRules
}

const settlerHookRules = `
## 伏笔追踪规则（严格执行）
...
const settlerOutputFormat = `## 输出格式（必须严格遵循）
